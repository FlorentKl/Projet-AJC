...
public class PlatRestController {
    ...
    public ResponseEntity<List<Plat>> findByDifficulte(@PathVariable("difficulte") String difficulte) {
        Difficulte diff;
        switch (difficulte) {
            case "F":
                diff = Difficulte.F;
        ...
        return new ResponseEntity<>(platService.findByDifficulte(diff), HttpStatus.OK);
    }
